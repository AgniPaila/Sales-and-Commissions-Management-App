...
public class SalesmanInformationWindow extends JDialog {
	...
	private LoadingSalesmanFromFileWindow inputDialog;
	private Salesman selectedSalesman;
	@SuppressWarnings("unused")
	private String fileType;
	
	public SalesmanInformationWindow(LoadingSalesmanFromFileWindow dialog, Salesman salesman, String fileTypeFlag) {
		inputDialog = dialog;
	...
	protected void okButtonPressed(ActionEvent evt) {
	    ...
	    shirtSales = shirtRadio.isSelected() ? selectedSalesman.calculateSales("Shirt") : -1;
	    skirtSales = skirtRadio.isSelected() ? selectedSalesman.calculateSales("Skirt") : -1;
	    coatsSales = coatRadio.isSelected() ? selectedSalesman.calculateSales("Coat") : -1;
	    trousersSales = trousersRadio.isSelected() ? selectedSalesman.calculateSales("Trouser") : -1;
	    commission = commissionCheckBox.isSelected() ? selectedSalesman.calculateCommission() : -1;
		
		SalesExtractionWindow rs = new SalesExtractionWindow(this,selectedSalesman, totalSales, totalItems, shirtSales, skirtSales, trousersSales, coatsSales, commission);
		rs.setVisible(true);
	...
	private void addReceiptButtonPressed(ActionEvent evt) {
		if(areReceiptFieldsEmpty()){
			...
			addReceipt();
			appendFile(selectedSalesman.getReceipts().get(selectedSalesman.getReceipts().size()-1));
	...
	private boolean areReceiptFieldsEmpty() {
	...
	private void appendFile(Receipt receipt){    
		selectedSalesman.getFileAppender().appendAReceipt(receipt);
