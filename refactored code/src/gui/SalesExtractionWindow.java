...
import output.HTMLReportGeneration;
import output.TXTReportGeneration;
import output.XMLReportGeneration;
...
public class SalesExtractionWindow extends JDialog {
	...
	private SalesmanInformationWindow salesmanInformationWindow;
	...
	public SalesExtractionWindow(final SalesmanInformationWindow sw, Salesman salesman,double tSales,int tItems,
			float shirtS,float skirtS,float trousersS,float coatsS,double com) {
		salesmanInformationWindow = sw;
	...
	private void initialise(){
		...
		resultWindowPanel.setLayout(null);
		{
			...
			xmlReportButton.setBounds(436, 140, 163, 84);
		...
		}
		{
			...
			htmlReportButton.setBounds(436, 250, 163, 88);
	...
	private void outputTXTButtonPressed(ActionEvent evt) {
		TXTReportGeneration makeTXTFile = new TXTReportGeneration(selectedSalesman);
		
		JFileChooser fileChooser = new JFileChooser();
		 
        JFrame parentComponent = new JFrame("Save");
		int status = fileChooser.showSaveDialog(parentComponent);
        ...
        if (status == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                ...
                makeTXTFile.generateAReport(fileName);
	...
	private void outputXMLButtonPressed(ActionEvent evt) {
		XMLReportGeneration makeXMLFile = new XMLReportGeneration(selectedSalesman);
        ...
        if (status == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                String fileName = selectedFile.getCanonicalPath();
                System.out.println(fileName);
                makeXMLFile.generateAReport(fileName);
	...
	private void outputHTMLButtonPressed(ActionEvent evt) {
		HTMLReportGeneration makeHTMLFile = new HTMLReportGeneration(selectedSalesman);
        ...
        if (status == JFileChooser.APPROVE_OPTION) {
            ...
            try {
                String fileName = selectedFile.getCanonicalPath();
                System.out.println(fileName);
                makeHTMLFile.generateAReport(fileName);
	...
	private void cancelButtonPressed(ActionEvent evt) {
		salesmanInformationWindow.setVisible(true);
		this.dispose();	
	}
