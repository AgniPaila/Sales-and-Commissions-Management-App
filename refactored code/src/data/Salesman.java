...
import output.ReceiptAppenderInTXTFile;
import output.ReceiptAppenderInXMLFile;

public class Salesman {
	private String name;
	private String afm;
	private ArrayList<Receipt> allReceipts;
	private ReceiptAppender receiptAppender;
	...
	public void setFileType(String fileType) {
		if(fileType.equals("TXT")){
			receiptAppender = new ReceiptAppenderInTXTFile();
		}	
		else{
			receiptAppender = new ReceiptAppenderInXMLFile();
		}	
	}
	...
	public float calculateSales(String kind) {
		float totalNumberOfAClothingKind = 0; 
		for (int i = 0; i< allReceipts.size(); i++){
			if(allReceipts.get(i).getKind().equals(kind)){
				totalNumberOfAClothingKind += allReceipts.get(i).getItems();
			}
		}
		return totalNumberOfAClothingKind;
	}
	...
	public ReceiptAppender getFileAppender() {
		return receiptAppender;
	}
